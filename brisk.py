# -*- coding: utf-8 -*-
"""
Created on Mon Sep  4 15:52:42 2023
...
cv2.imshow('BRISK: Regular Image', img3)
...
kp_90_clkw = brisk.detectAndCompute(img_90_clkw, None)
# compute the descriptors with BRIEF
kp_90_clkw, desc_90_clkw = brisk.detectAndCompute(img_90_clkw, None)

matches = bf.match(desc1, desc_90_clkw)
matches = sorted(matches, key=lambda x: x.distance)
end = datetime.now()

img3_90_clkw = cv2.drawMatches(img1, kp1, img_90_clkw, kp_90_clkw, matches[300:600], img2, flags=2)
cv2.imshow('BRISK: 90 degree clockwise Rotated Image', img3_90_clkw)
cv2.waitKey(0)
td = (end - start).total_seconds() * 10**3
...
# find the keypoints with STAR for clockwise 90 degree rotated image
kp_180_clkw = brisk.detectAndCompute(img_180_clkw, None)
# compute the descriptors with BRIEF
kp_180_clkw, desc_180_clkw = brisk.detectAndCompute(img_180_clkw, None)

matches = bf.match(desc1, desc_180_clkw)
matches = sorted(matches, key=lambda x: x.distance)
end = datetime.now()

img3_180_clkw = cv2.drawMatches(img1, kp1, img_90_clkw, kp_180_clkw, matches[300:600], None, flags=2)
cv2.imshow('BRISK: 180 degree clockwise Rotated Image', img3_180_clkw)
cv2.waitKey(0)
td = (end - start).total_seconds() * 10**3
...
# find the keypoints with STAR for clockwise 90 degree rotated image
kp_270_clkw = brisk.detectAndCompute(img_270_clkw, None)
# compute the descriptors with BRIEF
kp_270_clkw, desc_270_clkw = brisk.detectAndCompute(img_270_clkw, None)

matches = bf.match(desc1, desc_270_clkw)
matches = sorted(matches, key=lambda x: x.distance)
end = datetime.now()

img3_270_clkw = cv2.drawMatches(img1, kp1, img_270_clkw, kp_270_clkw, matches[300:600], None, flags=2)
cv2.imshow('BRISK: 90 degree anti-clockwise Rotated Image', img3_270_clkw)
cv2.waitKey(0)
td = (end - start).total_seconds() * 10**3
